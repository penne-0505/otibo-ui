...
const TEST_MATRIX_STATUS_VALUES = ["planned", "covered", "verified", "deferred", "not-applicable"]
...
const hasSubstantiveContent = (content) => {
  ...
  const withoutEmptyTables = stripCodeBlocks(content)
    ...
    .filter((line) => {
      ...
      if (/^\|\s*(Command \/ Test|Checklist Item|ID)\s*\|\s*(Result|Reason)\s*\|/i.test(line)) {
        return false
      }
      return true
    })
...
const validateFrontMatter = (file, attrs, errors) => {
  ...
  if (attrs.qa_status && !QA_STATUS_VALUES.includes(attrs.qa_status)) {
    add(errors, file, `qa_status must be one of ${QA_STATUS_VALUES.join(", ")}`)
  }
...
const validateTestMatrix = (file, src, errors, warnings) => {
  ...
  for (const row of rows) {
    ...
    if (status && !TEST_MATRIX_STATUS_VALUES.includes(status)) {
      add(errors, file, `Test Matrix status must be one of ${TEST_MATRIX_STATUS_VALUES.join(", ")}`)
    }
    if (status === "deferred") {
...
const validateTestPlan = async ({ file, src, attrs, area, slug, errors, warnings }) => {
  const usesWhyFirstSchema = attrs.qa_schema === QA_SCHEMA

  if (!["planned", "in-progress"].includes(attrs.qa_status)) {
    add(errors, file, "test-plan qa_status must be planned or in-progress")
  }
  ...
  if (usesWhyFirstSchema && !sectionHasId(src, "Decision Review Scope", "DEC")) {
  ...
  if (usesWhyFirstSchema && !isExplicitNone(invariants) && !/\bINV-\d{3}\b/.test(invariants)) {
    add(errors, file, "Intent-derived Invariants must be None or include INV-001 style IDs")
  } else if (!usesWhyFirstSchema && !/\bINV-\d{3}\b/.test(invariants)) {
    add(errors, file, "Intent-derived Invariants must include INV-001 style IDs")
  }
...
const validateVerification = ({ file, src, attrs, area, slug, errors }) => {
  const usesWhyFirstSchema = attrs.qa_schema === QA_SCHEMA
  const requiredHeadings = [
    "Summary",
  ...
  const verdict = verdictSection.match(/\bVerdict:\s*(PASS|PARTIAL|FAIL|BLOCKED)\b/)?.[1]
  ...
  if (!["verified", "partial", "failed", "blocked"].includes(attrs.qa_status)) {
    add(errors, file, "verification qa_status must be verified, partial, failed, or blocked")
  }
  ...
  if (verdict === "PASS" && !isNoneLike(residual)) {
    add(errors, file, "PASS verification must have empty or None residual risks")
  }
  if (
    ...
    add(errors, file, "PARTIAL/FAIL/BLOCKED requires residual risks or follow-up TODOs")
  }
  ...
  if (usesWhyFirstSchema && !sectionHasId(src, "Decision Conformance", "DEC")) {
  ...
  } else if (!usesWhyFirstSchema && !sectionHasId(src, "Invariant Coverage", "INV")) {
  ...
  if (
    ...
    add(errors, file, "Commands Run or Manual QA Results must contain substantive evidence")
  }
}
...
const parseTodoTasks = (src) => {
  ...
  for (const line of stripped.split(/\r?\n/)) {
    ...
    if (currentField && (/^\s+/.test(line) || line.trim() === "")) {
      current.fields[currentField] = `${current.fields[currentField] ?? ""}\n${line}`.trimEnd()
    }
  }
  flush()
  return tasks
  ...
  for (const task of tasks) {
    ...
    for (const field of ["QA", "Verification"]) {
      ...
      if (!(await exists(path))) {
        add(errors, TODO_FILE, `${label}: ${field} file does not exist: ${path}`)
        continue
      }
      ...
      if (intent !== "None" && !referencesInclude(attrs, intent)) {
        add(errors, path, `${field} references must include TODO Intent ${intent}`)
      }
...
const effectiveQaMatch = ({ file, src, attrs, fixtureMode, errors }) => {
  const effectivePath =
    fixtureMode && typeof attrs.fixture_path === "string" ? normalizePath(attrs.fixture_path) : file
  if (effectivePath.includes("_docs/archives/qa/") || /(^|\/)archives\/qa\//.test(effectivePath)) {
  ...
  if (!fixtureMode) {
    add(errors, file, "QA path must match _docs/qa/<Area>/<slug>/test-plan.md or verification.md")
    return null
  }

  const kind = sectionContent(src, "Verification Verdict") === null ? "test-plan" : "verification"
