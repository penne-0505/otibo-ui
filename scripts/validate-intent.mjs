...
const decisionEntries = (content) => {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (!match) continue
    const end = lines.findIndex((line, index) => index > i && /^###\s+DEC-\d{3}:/.test(line))
    entries.push({
      id: match[1],
      title: match[2].trim(),
...
const validateV2 = (file, src, errors) => {
  ...
  if (invariantLines.length === 0) {
    add(errors, file, "Intent-derived Invariants must be None or use INV-001 (from DEC-001)")
    return
  }

  const invariantIds = new Set()
  for (const line of invariantLines) {
    const match = line.match(/^- (INV-\d{3}) \(from (DEC-\d{3})\):\s*(.+)$/)
    if (!match) {
      add(errors, file, `invalid invariant format: ${line}`)
      continue
    }
    ...
    if (!decisionIds.has(decisionId)) {
      add(errors, file, `${invariantId} references missing decision ${decisionId}`)
    }
  }
}
...
const run = async () => {
  ...
  const fixtureTarget = fixtureIndex === -1 ? null : Deno.args[fixtureIndex + 1]
  if (fixtureIndex !== -1 && !fixtureTarget) {
    console.error("ERROR: --fixture requires a file or directory")
    Deno.exit(1)
  }

  const target = fixtureTarget ?? Deno.args.find((arg) => !arg.startsWith("-")) ?? "_docs/intent"
  ...
  for (const file of await collectFiles(target)) {
    ...
    const logicalPath = typeof attrs.fixture_path === "string" ? attrs.fixture_path : file
    ...
    if (attrs.intent_schema !== INTENT_SCHEMA) {
      add(errors, file, `intent_schema must be ${INTENT_SCHEMA} when present`)
      continue
    }
