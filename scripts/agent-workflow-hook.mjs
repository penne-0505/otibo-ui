...
const HOOK_EVENT_NAMES = new Set(["SessionStart", "Stop", "PreToolUse", "UserPromptSubmit"])
...
// intent: DEC-006 (Workflow/docs-template-v1-migration) — Prompt hooks remain short guardrails and do not expand task authority.
...
// intent: DEC-006 (Workflow/docs-template-v1-migration) — Write and stop audits check consequences without authorizing broader work.
...
export const parsePorcelainPaths = (statusOutput) => {
  ...
  for (const rawLine of String(statusOutput ?? "").split(/\r?\n/)) {
    ...
    if (body.includes(" -> ")) {
      const [from, to] = body.split(" -> ").map((value) => value.replace(/^"|"$/g, ""))
...
const pathFromToolInput = (toolInput) => {
  ...
  return normalizePath(toolInput.file_path ?? toolInput.path ?? toolInput.target_file ?? "")
}
...
const includesSensitivePath = (value) =>
  /(^|[/\s'"`])(\.env(\.|$|[/\s'"`])|id_rsa\b|id_ed25519\b|\.pem\b|\.key\b)/i.test(
    String(value ?? ""),
  )

const protectedArchiveMove = (command) =>
  /\b(git\s+)?mv\b[\s\S]*_docs\/(intent|qa|guide|reference)\b[\s\S]*_docs\/archives\b/i.test(
    command,
  ) || /\b(git\s+)?mv\b[\s\S]*_docs\/archives\/(intent|qa|guide|reference)\b/i.test(command)
...
const destructiveCommandReason = (command) => {
  ...
  const checks = [
    ...
    },
    {
      ...
      reason: "git reset --hard is blocked because it can discard user or agent work.",
    },
    {
      re: /(^|[;&|()\s])git\s+clean(\s|$)/,
    ...
    },
    {
      ...
      reason: "git checkout -- is blocked because it can discard local changes.",
    },
  ]
...
const relevantStopPaths = (paths) => {
  ...
  return {
    ...
    docs: normalized.filter(
      (path) =>
        path.startsWith("_docs/") ||
        ["README.md", "QUICKSTART.md", "AGENTS.md", "CLAUDE.md"].includes(path),
    ),
    workflow: normalized.filter(
      (path) =>
        path === "AGENTS.md" ||
        path === "CLAUDE.md" ||
    ...
    temporaryDocs: normalized.filter((path) => /^_docs\/(draft|plan|survey)\//.test(path)),
...
export const auditEvidenceCount = (message) =>
  AUDIT_EVIDENCE_PATTERNS.filter((pattern) => pattern.test(message ?? "")).length
...
export const analyzeStop = ({ input = {}, dirtyPaths = [] }) => {
  ...
  const hasRelevantChange =
    grouped.todo.length > 0 || grouped.docs.length > 0 || grouped.workflow.length > 0
