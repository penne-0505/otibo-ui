...
const isStringArray = (val) => Array.isArray(val) && val.every((v) => typeof v === "string")
const isIntegerArray = (val) => Array.isArray(val) && val.every((v) => Number.isInteger(v))
...
const diffDays = (from, to) => Math.floor((to.getTime() - from.getTime()) / MS_PER_DAY)
...
const run = async () => {
  ...
  for await (const file of walkMarkdown("_docs")) {
    ...
    if (!isIntegerArray(data.related_issues)) {
      fileErrors.push("related_issues must be an array of integers (can be empty)")
    }
    ...
    if (isQaPath(file)) {
      if (!("qa_status" in data)) {
        ...
        fileErrors.push(`qa_status must be one of ${QA_STATUS_VALUES.join(", ")}`)
      }
    ...
    if (staleExemptUntilRaw !== undefined) {
      ...
      if (!parsed) {
        ...
        fileErrors.push("stale_exempt_until must not be earlier than updated_at")
      }
    }
    if (staleExemptReason !== undefined && typeof staleExemptReason !== "string") {
    ...
    if (staleExtensions !== undefined && !isNonNegativeInt(staleExtensions)) {
      fileErrors.push("stale_extensions must be a non-negative integer when provided")
    }
    ...
    if (isDraftPath(file) && status === "proposed" && updatedAt) {
      ...
      if (daysSinceUpdate > STALE_DAYS) {
        const parsedExempt = staleExemptUntilRaw ? parseDate(staleExemptUntilRaw) : null
        if (!parsedExempt || parsedExempt < today) {
          fileErrors.push(
            `draft is stale (${daysSinceUpdate} days since updated_at) without valid stale_exempt_until`,
          )
        }
        if (staleExemptUntilRaw && (!staleExemptReason || staleExemptReason.trim() === "")) {
          fileErrors.push("stale_exempt_reason is required when stale_exempt_until is set")
        }
      } else if (staleExemptUntilRaw) {
        fileWarnings.push("stale_exempt_until is set but draft is not stale yet (<=30 days)")
      }
    }
