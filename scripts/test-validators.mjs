...
const TODO_VALID = ["_evals/validator-fixtures/todo/valid/basic.md"]
const TODO_INVALID = [
...
const INTENT_VALID = ["_evals/validator-fixtures/intent/valid"]
...
const QA_VALID = ["_evals/validator-fixtures/qa/valid"]
const QA_INVALID = [
...
const validatorArgs = (kind, target) => {
  ...
  if (kind === "intent") {
    return ["run", "--allow-read", "scripts/validate-intent.mjs", "--fixture", target]
  }
  return ["run", "--allow-read", "scripts/validate-qa.mjs", "--fixture", target]
}
...
const testCase = async ({ kind, target, shouldPass }) => {
  ...
  if (passed) {
    console.log(shouldPass ? `PASS ${label}` : `PASS ${label} failed as expected`)
    return true
  }
...
const SCOPE_FIXTURE = "_evals/validator-fixtures/qa/invalid/missing-invariant.md"
...
const runFrontmatterFixtureCase = async ({
  ...
  try {
    ...
    const command = new Deno.Command(deno, {
      args: ["run", "--allow-read", `${repoRoot}/scripts/validate-frontmatter.mjs`],
      cwd: temp,
      stdout: "piped",
      stderr: "piped",
    })
...
const scopeCase = async ({ label, scopePaths, shouldPass }) => {
  ...
  console.error(`FAIL scope ${label}: exit ${code} (expected ${shouldPass ? "0" : "non-zero"})`)
  return false
}
...
ok =
  (await runFrontmatterFixtureCase({
...
ok =
  (await runFrontmatterFixtureCase({
    ...
    fixture: "_evals/validator-fixtures/frontmatter/warning/schema-on-wrong-type.md",
    target: "_docs/reference/Core/schema-fixture/reference.md",
    expectedWarnings: ["unknown field: intent_schema", "unknown field: qa_schema"],
  })) && ok
ok =
  (await runFrontmatterFixtureCase({
