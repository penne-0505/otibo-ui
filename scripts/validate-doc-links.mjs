...
const ROOT_RELATIVE_PREFIXES = ["_docs/", "_evals/", ".agents/", ".claude/", ".github/", "scripts/"]
const ROOT_RELATIVE_FILES = ["README.md", "AGENTS.md", "TODO.md", "QUICKSTART.md", "LICENSE.txt"]
...
const collectIntentReferences = async () => {
  const refs = new Set()
  for await (const file of walkFiles("_docs/intent", (path) => path.endsWith(".md"))) {
...
const validateArchiveInvariants = async (errors) => {
  for (const type of FORBIDDEN_ARCHIVE_TYPES) {
    ...
    if (await isDirectory(dir)) {
      errors.push({
        file: dir,
        message: "archive directories are only allowed for draft, plan, and survey",
      })
    }
  }

  const intentRefs = await collectIntentReferences()
  for await (const file of walkFiles("_docs/archives", (path) => path.endsWith(".md"))) {
    const parts = file.split("/")
    const [, archives, type, area, slug] = parts
    if (archives !== "archives" || !ARCHIVE_TYPES.includes(type) || !area || !slug) {
      errors.push({
        file,
        message: "archive path must match _docs/archives/{draft,plan,survey}/<Area>/<slug>/...",
      })
      continue
    }
    ...
    if (await hasMarkdown(liveDir)) {
      errors.push({
        file,
        message: `live ${type} document still exists for archived ${area}/${slug}`,
      })
    }
  }
}
...
const validateQaInvariants = async (errors) => {
  for await (const file of walkFiles("_docs/qa", (path) => path.endsWith(".md"))) {
    ...
    if (!match) {
      errors.push({
        file,
        message: "QA path must match _docs/qa/<Area>/<slug>/test-plan.md or verification.md",
      })
      continue
    }
...
const validateGuideReferenceWarnings = async (warnings) => {
  for (const type of ["guide", "reference"]) {
    for await (const file of walkFiles(`_docs/${type}`, (path) => path.endsWith(".md"))) {
      const src = await Deno.readTextFile(file)
      const { attrs } = parseFrontMatter(src)
      if (attrs?.status !== "active") continue
      const refs = Array.isArray(attrs.references) ? attrs.references : []
      const hasRelatedRef = refs.some(
        (ref) =>
          typeof ref === "string" &&
          (ref.includes("/intent/") ||
            ref.includes("../intent/") ||
      ...
      if (!hasRelatedRef) {
        warnings.push({
          file,
          message: "active guide/reference should reference related intent, guide, or reference",
        })
      }
    }
  }
