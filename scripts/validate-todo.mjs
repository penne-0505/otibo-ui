...
const CATEGORIES = ["Feat", "Enhance", "Bug", "Refactor", "Perf", "Doc", "Test", "Chore"]
...
const ID_RE = new RegExp(`^([A-Za-z][A-Za-z0-9-]*)-(${CATEGORY_PATTERN})-([1-9]\\d*)$`)
const TASK_HEADING_RE = new RegExp(
  `^###\\s+([A-Za-z][A-Za-z0-9-]*-(${CATEGORY_PATTERN})-[1-9]\\d*):\\s+\\[(${CATEGORY_PATTERN})\\]\\s+(.+?)\\s*$`,
)
const FIELD_RE = /^-\s+(?:\*\*([A-Za-z][A-Za-z ]*)\*\*|([A-Za-z][A-Za-z ]*)):\s*(.*)$/
const PATH_PATTERNS = {
  Plan: /^_docs\/plan\/([A-Za-z][A-Za-z0-9-]*)\/([a-z0-9]+(?:-[a-z0-9]+)*)\/plan\.md$/,
  Intent: /^_docs\/intent\/([A-Za-z][A-Za-z0-9-]*)\/([a-z0-9]+(?:-[a-z0-9]+)*)\/decision\.md$/,
  QA: /^_docs\/qa\/([A-Za-z][A-Za-z0-9-]*)\/([a-z0-9]+(?:-[a-z0-9]+)*)\/test-plan\.md$/,
  Verification:
...
const normalizeSectionName = (value) => value.replace(/\s*\(.*\)\s*$/, "").trim()
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (heading) {
      ...
      if (headingMatch) {
        ...
        current.headingError = "task heading must match ### <ID>: [<Category>] <Title>"
      }
      continue
    }
...
const riskAtLeast = (risk, floor) => RISKS.indexOf(risk) >= RISKS.indexOf(floor)

const isNone = (value) => normalizeInlineCode(value ?? "") === "None"

const validateDocPath = async ({ task, label, field, value, mustExist, errors }) => {
  const normalized = normalizeInlineCode(value ?? "")
...
const taskLabel = (task) =>
  task.headingId ?? task.fields.ID ?? task.fields.Title ?? `line ${task.lineNo}: ${task.heading}`
...
const run = async () => {
  ...
  for (const task of tasks) {
    ...
    if (!parsedTitle) {
      ...
      add(errors, `${label}: Title category must be one of ${CATEGORIES.join(", ")}`)
    }
    ...
    if (deps === null) {
      ...
      for (const dep of deps) {
        ...
        } else if (!ids.has(dep) && !tasks.some((other) => other.fields.ID === dep)) {
          add(warnings, `${label}: dependency is not in current TODO.md: ${dep}`)
        }
      }
    }
    ...
    if (acIds.length === 0) {
      add(errors, `${label}: Acceptance Criteria must include AC-001 style IDs`)
    }
    ...
    if (
      category === "Bug" &&
      !/regression|prevent|再発|回帰|no-test/i.test(task.fields["Acceptance Criteria"] ?? "")
    ) {
      add(
        warnings,
