...
const contains = (text, ...needles) => needles.every((needle) => text.includes(needle))
...
const agentsMigration = await read(".agents/skills/docs-template-migration/SKILL.md")
const claudeMigration = await read(".claude/skills/docs-template-migration/SKILL.md")
...
const documentationOperations = await read("_docs/standards/documentation_operations.md")
...
assert(agentsCleanup === claudeCleanup, "docs-cleanup skill is synced across .agents and .claude")
...
const migrationSteps = agentsMigration.split(/^### \d+\..*$/m).slice(1)
assert(
...
assert(
  templateLockExample.schema === 1 &&
    templateLockExample.source === "https://github.com/penne-0505/docs_driven_dev_template.git" &&
    templateLockExample.revision?.tag === "v1.0.0" &&
    templateLockExample.revision?.commit === "REPLACE_WITH_THE_TAGS_FULL_40_CHARACTER_COMMIT_SHA",
...
assert(
  contains(intentTemplate, "intent_schema: 2", "### DEC-001:", "**Why**:", "**Change freedom**:"),
...
assert(
  contains(qualityStandard, "INV が 0 件でも正常", "exact 値を固定するテスト"),
