...
> Implementation: `src/core-ui/segmented-control/`

## Overview

**密に並ぶ「一つだけ選ぶ」設定**(連結 pill + 滑る indicator)。radio / select と同じ「一つ選ぶ」だが、選択肢が少なく(2〜4 個)即時に切替える設定向け。実装は **Base UI Tabs を primitive に流用** ──「凹んだ溝に白い pill が滑る」表現で、active 位置の追従を indicator slot に委譲する。Toggle(四角・toolbar)/ Chip(角丸・フィルタ複数)とは見た目と用途で住み分け。
...
```tsx
<SegmentedControlRoot value={theme} onValueChange={setTheme}>
...
- **primitive** ── `@base-ui/react/tabs`の root / list / indicator / tab slotを流用。
...
- **primitive = Base UI Tabs を流用**(radio / select でなく) ── indicator slotの **active 位置自動追従** が「滑る pill」の実装の核。radiogroup primitive を使うと indicator 自動追従が無く、CSS で位置計算を手で組む必要がある。a11y は妥協(tab role が読まれる)を許容して grain を取った。
