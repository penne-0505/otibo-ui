...
Resolve and compare upstream revisions in an explicit checkout of the upstream
`docs_driven_dev_template` repository, never in the downstream project:

```bash
git -C /path/to/docs_driven_dev_template rev-parse 'refs/tags/<tag>^{}'
git -C /path/to/docs_driven_dev_template rev-parse <recorded-full-sha>
```
